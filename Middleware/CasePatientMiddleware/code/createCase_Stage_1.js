const casePatinetCreateStage1Middleware = async (req, res, next) => {
    ...
    try {
        ...
        if (typeof payload._data_stage_1 != 'object'){ ErrorJson.description.push(`Paratmer <_data_stage_1> mest be Object`); }
        if (typeof payload._data_stage_1._ref_casepatinetid != 'string' || payload._data_stage_1._ref_casepatinetid == '' || payload._data_stage_1._ref_casepatinetid != payload._casepatientid) { 
            ErrorJson.description.push(
                `casePatinetCreateStage1Controller: <_data_stage_1._ref_casepatinetid> must be String and Not Empty and _data_stage_1._ref_casepatinetid === _casepatientid`
            ); 
        }
        if (typeof payload._data_stage_1.update_congenital_disease != 'boolean') { ErrorJson.description.push(`Paratmer <_data_stage_1.update_congenital_disease> must be Boolean`); }
        if (typeof payload._data_stage_1.update_drug_allergy != 'boolean') { ErrorJson.description.push(`Paratmer <_data_stage_1.update_drug_allergy> must be Boolean`); }
        if (typeof payload._data_stage_1.update_illness_history != 'boolean') { ErrorJson.description.push(`Paratmer <_data_stage_1.update_illness_history> must be Boolean`); }
...
module.exports = casePatinetCreateStage1Middleware;