...
const schedule_Cancel_Middleware = async (req, res, next) => {
    ...
    try {
        ...
        if (!miscController.validateObjectId(payload.scheduleid)) { ErrorJson.description.push(`Paratmer <storeid> must be ObjectId String and Not Empty`); }
        ...
        if (!miscController.validateObjectId(payload.storeid)) { ErrorJson.description.push(`Paratmer <branchid> must be ObjectId String and Not Empty`); }        
