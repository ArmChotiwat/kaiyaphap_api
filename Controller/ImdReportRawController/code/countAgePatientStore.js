...
// จำนวนผู้ป่วยแต่ละช่วงอายุ (ตาม StoreId)
const countAgePatientStoreController = async (data = { _storeid: new String }, callback = (err = new Error) => {} ) => {
  ...
  const patientModel = require('../../mongodbController').patientModel;
    ...
    try {
        ...
        const getReportNull = await patientModel.aggregate(
          [
            {
              '$match': {
                'store._storeid': _storeid
        ...
        let countAgeUndefined = 0;
        if(getReportNull.length === 1) { countAgeUndefined = getReportNull[0].nullCount }
        ...
        const mapData = {
            ...
            "AgeUndefined": countAgeUndefined || 0
        }
...
module.exports = countAgePatientStoreController;