const reportRaw_listCountIllnessCharacticsPatient = async (data = { _storeid: new String, _branchid: new String, month: new String, year: new String }, callback = (err = new Error) => {} ) => {
    ...
    const illnessCharacticModel = require('../../mongodbController').illnessCharacticModel;
    ...
    try {
        const illdata = await scheduleModel_Refactor.aggregate(
          [
            {
              ...
              '$project': {
                'illdata': '$data.store.personal.general_user_detail.congenital_disease'
              }
            }, {
              '$unwind': {
                'path': '$illdata'
          ...
          if(illdata.length < 0) { callback(new Error('reportRawController => reportRaw_listCountIllnessCharacticsPatient: schedule Model Error')); return; }
          else {
            ...
            const illmaster = await illnessCharacticModel.find({'_storeid': _storeid, 'isused': true});
...
module.exports = reportRaw_listCountIllnessCharacticsPatient;