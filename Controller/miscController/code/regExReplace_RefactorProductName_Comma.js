/**
 * Function => regExReplace_RefactorProductName_comma
 * 
 ...
 * (สวัสดี , ) => (สวัสดี,)
 */
const regExReplace_RefactorProductName_Comma = (stringdata = new String('')) => {
    stringdata = stringdata.replace(new RegExp(' +\,', 'g'), ','); // Refactor Whitespace if Detect ,
    stringdata = stringdata.replace(new RegExp('\, +', 'g'), ', '); // Refactor Whitespace if Detect ,
    return stringdata;
};

module.exports = regExReplace_RefactorProductName_Comma;