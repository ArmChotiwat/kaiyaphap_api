...
const documentName = 'l_treatment';
const treatmentSchema = new Schema({
    ...
    medical_certificate_th: { // ใบรับรองแพทย์ (ภาษาไทย)
        ...
        agent_name: { type: String, default: null }, // นักกายภาพ (Free Text)
...
treatmentSchema.index({ '_ref_storeid': 1 }, {});
treatmentSchema.index({ '_ref_branchid': 1 }, {});
treatmentSchema.index({ '_ref_storeid': 1, '_ref_branchid': 1 }, {});

treatmentSchema.index({ '_ref_casepatinetid': 1 }, {});
treatmentSchema.index({ '_ref_storeid': 1, '_ref_branchid': 1, '_ref_casepatinetid': 1 }, {});

treatmentSchema.index({ '_ref_scheduleid': 1 }, { unique: true }); // 1 คิว ต่อ 1 ใบสั่งยา

treatmentSchema.plugin(updateIfCurrentPlugin, { strategy: "version" });

const treatmentModel = mongooseConn.model(documentName, treatmentSchema);
